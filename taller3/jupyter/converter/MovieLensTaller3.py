...
class MovieLensTaller3Converter:
    ...
    def __init__(self):
        ...
        self.path = {
            'ratings': os.path.join(here, '../data/ml-latest/ratings.csv'),
            'items': os.path.join(here, '../data/ml-latest/movies.csv')
        }
    ...
    def __load_movies(self):
		...
		all_movies=pd.read_csv(self.path['items'],sep=',', names=['movie_id','title','genres'], header=1)
		all_genres = ['Action',
					'Adventure',
					'Animation',
					"Children",
